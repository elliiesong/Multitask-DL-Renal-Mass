...
import os
import argparse
from dataloader import randomData
import se_resnet
        ...
        for phase in ['train', 'val']:
            ...
            for i, (inputs, labels, ex_labels, file) in enumerate(dataloders[phase]):
                ...
                septa_loss = criterion.get_ex_loss(outputs[:, 3:6], septa,
                                                   weight=[1.65, 4.71, 5.508])  # [2.426, 3.945, 2.992]
                septa_tn_loss = criterion.get_ex_loss(outputs[:, 6:10], septa_tn,
                                                      weight=[1.65, 7.065, 12.5, 5.804])  # [3.663, 6.527, 4.174, 2.992]
                wall_tn_loss = criterion.get_ex_loss(outputs[:, 10:14], wall_tn,
                                                     weight=[1.195, 25, 10.484, 36.111])  # [6.904, 7.18, 13.296, 1.561]
                nodule_loss = criterion.get_ex_loss(outputs[:, 14:16], nodule,
                                                    weight=[1.012, 81.25])  # [1.023, 44.875]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-path', type=str, default=r"output\random\step2")
    ...
    parser.add_argument('--val-times', '-vt', type=int, default=100)
    ...
    acc = np.zeros(args.val_times)
    for i in range(args.val_times):
        print(f'fold{i} start')
        args = parser.parse_args(['--save-path', rf'output\random\step2\rd{i}'])
        # read data
        dataloders, dataset_sizes, w1, w2 = randomData(args)
        # get model
        ...
        criterion = loss_fn(label_weight=w2)  # [2.821, 3.437, 2.821]
    ...
    mean_acc = acc.mean()
    print(f'total acc: {mean_acc}')
    text = f'classes: {args.num_class}, batch_size: {args.batch_size}, epoch: {args.num_epochs}, lr: {args.lr}, val times: {args.val_times}, acc: {mean_acc}'
    with open(r'./output/log.txt', mode='a', encoding='utf-8') as f:
        f.write(text)
        f.write('\n')