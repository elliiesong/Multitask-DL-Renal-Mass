...
import os
import argparse
from dataloader import SliceData
import se_resnet
    ...
    for epoch in range(args.start_epoch+1, num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for i, (inputs, labels, ex_labels, file) in enumerate(dataloders[phase]):
                ...
                if use_gpu:
                    ...
                    septa, septa_tn, wall_tn, nodule, calcification =\
                        Variable(septa), Variable(septa_tn), Variable(wall_tn), Variable(nodule), Variable(calcification)
            ...
            if phase == 'train':
                ...
                # l_loss_list.append(epoch_l_loss)
                # s_loss_list.append(epoch_s_loss)
            # if phase == 'train':
            #     loss_list.append(epoch_loss)
                ...
                if epoch > 30:
                    if mean_acc > 0.5:
                        ...
                        if mean_acc <= best_accuracy:
                            ...
                            plot_confusion_matrix(conf_matrix.numpy(), accuracy=epoch_acc, classes=classes, normalize=False, title='Confusion_matrix')
...
if __name__ == '__main__':
    ...
    # criterion = LabelSmoothLoss(smoothing=0.26)
    # criterion = FocalLoss(alpha=[3.316, 8.523, 6.423, 8.071, 3.316]) # 5 labels
    # criterion = FocalLoss(alpha=[2.942, 3.123, 2.942]) # step1
    # criterion = FocalLoss(alpha=[3.336, 2.587, 3.188]) # step2
    criterion = loss_fn(label_weight=[1.676, 5.174, 4.76])  # [2.821, 3.437, 2.821]
    # criterion = torch.nn.CrossEntropyLoss(weight=torch.from_numpy(np.array([0.25, 0.25, 0.75, 0.75, 0.25])).float(),
